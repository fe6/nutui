...
export default create({
  ...
  setup(props, { emit, slots }) {
    ...
    const getParentElementH5 = (el: HTMLElement) => {
      return !!props.containerId ? document.querySelector(`#${props.containerId}`) : el && el.parentNode;
    };

    onMounted(() => {
      if (Taro.getEnv() === 'WEB') {
        const parentElement = getParentElementH5(state.scroller as HTMLElement) as Node & ParentNode;
    ...
    onActivated(() => {
      if (Taro.getEnv() === 'WEB' && isKeepAlive.value) {
