...
| show-index`v0.1.0`   | Whether to show index    | Boolean  | true  |
| closeable`v0.1.0`   | Whether to show close icon    | Boolean  | false  |
| close-icon`v0.1.0`   | Close icon name    | String  | ‘circle-close’  |
| close-icon-position`v0.1.0`   |  Close icon position,can be set to `top-left`   | String  | ‘top-right’  |  
| before-close`v0.1.0`  | Callback function before close   | (active: number) => boolean | Promise<`boolean`>  | -  | 
| max-zoom`v0.1.0`  | Max zoom`Taro isn't supported`   | Number  | 3  | 
| min-zoom`v0.1.0`  | Min zoom`Taro isn't supported`   | Number  | 1/3  | 
...
|change`v0.1.0`|Emitted when current image changed|无|
