...
| show-index`v0.1.0`  | 是否显示页码    | Boolean  | true  |
| closeable`v0.1.0`  | 是否显示关闭图标    | Boolean  | false  |
| close-icon`v0.1.0`   | 关闭图片名称或图片链接    | String  | ‘circle-close’  |
| close-icon-position`v0.1.0`   | 关闭图标位置，可选值：top-left   | String  | ‘top-right’  |  
| before-close`v0.1.0`  | 关闭前的回调函数，返回 false 可阻止关闭，支持返回 Promise   | (active: number) => boolean | Promise<`boolean`>  | -  | 
| max-zoom`v0.1.0`  | 手势缩放时，最大缩放比例`小程序暂不支持`   | Number  | 3  | 
| min-zoom`v0.1.0`  | 手势缩放时，最小缩放比例`小程序暂不支持`   | Number  | 1/3  | 
...
|change`v0.1.0`|切换图片时触发| index:当前图片索引|
